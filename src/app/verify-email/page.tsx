...
export default function VerifyEmailPage() {
  ...
  const { refreshBusiness, userProfile } = useBusiness();
  const [isResending, setIsResending] = useState(false);
  const [isChecking, setIsChecking] = useState(false);
  const [statusMessage, setStatusMessage] = useState(
    'We sent a secure verification link to your inbox.'
  );
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const [pageState, setPageState] = useState<
    'idle' | 'resent' | 'checking' | 'verified' | 'notVerified'
  >('idle');
  ...
  const handleBusinessActivation = async (verifiedUserId: string) => {
    ...
    if (userProfile?.businessId) {
      ...
      await markUserEmailVerified(verifiedUserId);
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setStatusMessage(
        'We resent the verification link to your inbox. It may take a few seconds to arrive.'
      );
  ...
  const handleVerified = async () => {
    ...
    try {
      ...
      if (!refreshedUser.emailVerified) {
        setError(
          'Your email is not verified yet. Please open the verification link from your inbox.'
        );
        setPageState('notVerified');
        return;
      }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4 py-10 text-foreground">
        <div className="w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-card p-8 text-center">
          ...
          <p className="mt-4 max-w-xl mx-auto text-sm text-muted-foreground">
            We could not find an authenticated user session. Sign back in so we can confirm the
            verification state securely.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/sign-up-login-screen"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-primary/20 transition hover:bg-indigo-600"
            >
              Back to Login
              <ArrowRight size={16} />
            </Link>
        ...
        <div className="grid w-full gap-10 lg:grid-cols-[1.15fr_1fr]">
          <section className="relative overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white/90 p-10 shadow-card">
            ...
            <div className="mb-8 flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="mt-2 text-3xl font-semibold text-foreground">
                  Verify your email to activate your business dashboard
                </h1>
              </div>
            </div>
            ...
            <p className="max-w-xl text-sm leading-7 text-slate-700">
              ...
              <span className="font-semibold text-slate-900">{emailAddress}</span>. Once verified,
              your BizManage workspace will be activated and your dashboard access will unlock
              instantly.
            </p>
              ...
              <div className="flex items-center gap-3 rounded-3xl border border-slate-200 bg-white p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Encrypted link delivery for your business data.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-3xl border border-slate-200 bg-white p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Only verified owners can access the dashboard.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-10 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-slate-50 p-6 shadow-sm">
              <h2 className="text-sm font-semibold uppercase tracking-[0.35em] text-primary/80">
              ...
              <div className="mt-6 space-y-4">
                {steps.map((step, index) => (
                  <div key={step.label} className="flex items-center gap-4">
                    <div
                      className={`flex h-10 w-10 items-center justify-center rounded-2xl ${step.active ? 'bg-primary/10 text-primary ring-1 ring-primary/20' : 'bg-slate-200 text-slate-500'}`}
                    >
                      {index + 1}
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {index === 2
                          ? 'Open the link in your inbox to complete verification.'
                          : index === 3
                            ? 'Dashboard access unlocks after email verification.'
                            : 'Completed'}
                      </p>
                    </div>
                  </div>
          ...
          <section className="relative rounded-[2rem] bg-slate-100/70 p-8 shadow-card">
            ...
            <div className="relative rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-5">
                ...
                <div className="grid gap-4">
                  ...
                  <Link
                    href="/sign-up-login-screen"
                    className="inline-flex h-14 w-full items-center justify-center gap-2 rounded-3xl border border-slate-300 bg-white text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                  >
                    <ArrowRight size={16} />
                    Back to login
                  </Link>
            ...
            <div className="mt-8 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5 text-slate-700 shadow-sm">
              ...
              <p className="mt-3 text-sm leading-7">
                Your verification status is checked automatically every few seconds while this page
                is open. No reload is required once the email is confirmed.
              </p>
            </div>
          </section>
        </div>
