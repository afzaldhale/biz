    ...
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        if (editingCourseId) {
          ...
          setCourses((current) =>
            current.map((course) => (course.id === editingCourseId ? { ...course, ...nextCourse } : course)),
          );
  ...
  return (
    <div className="max-w-screen-2xl mx-auto space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-700 tracking-[0.24em] text-primary uppercase">Academy Courses</p>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="glass-card rounded-2xl border border-border p-5">
          <p className="text-xs font-700 tracking-wide text-muted-foreground uppercase">Total Courses</p>
          <p className="text-2xl font-700 text-foreground mt-2">{totals.courses}</p>
        </div>
        <div className="glass-card rounded-2xl border border-border p-5">
          <p className="text-xs font-700 tracking-wide text-muted-foreground uppercase">Pending Offerings</p>
          <p className="text-2xl font-700 text-foreground mt-2">{courses.filter((course) => !course.duration).length}</p>
        </div>
        <div className="glass-card rounded-2xl border border-border p-5">
          <p className="text-xs font-700 tracking-wide text-muted-foreground uppercase">Average Fee</p>
        ...
        <div className="glass-card rounded-2xl border border-border p-5">
          <p className="text-xs font-700 tracking-wide text-muted-foreground uppercase">Categories</p>
      ...
      <div className="glass-card rounded-2xl border border-border overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="px-5 py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Add a new course to build your curriculum.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-border bg-white">
                {paginatedCourses.map((course) => (
                  <tr key={course.id}>
                    ...
                    <td className="px-5 py-4 text-muted-foreground">{course.category || 'General'}</td>
                    ...
                    <td className="px-5 py-4 text-muted-foreground">{formatCurrency(course.fee)}</td>
                    <td className="px-5 py-4 space-x-2">
                      <button type="button" onClick={() => openForm(course)} className="btn-outline px-3 py-2 rounded-xl text-xs">
                        <Pencil size={14} />
                      </button>
                      <button type="button" onClick={() => handleDelete(course.id)} className="btn-outline px-3 py-2 rounded-xl text-xs text-red-600 border-red-200 hover:bg-red-50">
                        <Trash2 size={14} />
                      </button>
                    </td>
                  </tr>
        ...
        {!loading && filteredCourses.length > 0 && (
          <div className="px-5 py-4 border-t border-border bg-white/80 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <p className="text-xs text-muted-foreground">
              Showing {(currentPage - 1) * rowsPerPage + 1}-{Math.min(currentPage * rowsPerPage, filteredCourses.length)} of {filteredCourses.length} courses
            </p>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 p-4 overflow-y-auto">
          <div className="mx-auto max-w-3xl rounded-3xl bg-white p-6 md:p-8 shadow-xl">
            <div className="flex items-center justify-between gap-4 mb-6">
              <div>
                <p className="text-sm text-muted-foreground">{editingCourseId ? 'Edit' : 'New'} Course</p>
                <h2 className="text-2xl font-700 text-foreground mt-1">{editingCourseId ? 'Update' : 'Create'} course</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="grid gap-5 md:grid-cols-2">
              ...
              <div>
                ...
                <input name="category" value={formValues.category} onChange={handleChange} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">Instructor</label>
                <input name="instructor" value={formValues.instructor} onChange={handleChange} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">Duration</label>
                <input name="duration" value={formValues.duration} onChange={handleChange} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" placeholder="e.g. 3 months / 5 weeks" />
              </div>
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">Fee</label>
                <input name="fee" type="number" min="0" value={formValues.fee} onChange={handleChange} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div className="md:col-span-2">
              ...
              <div className="md:col-span-2 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button type="submit" disabled={submitting} className="btn-primary px-5 py-3 rounded-xl text-sm disabled:opacity-60 disabled:cursor-not-allowed">{submitting ? 'Saving...' : editingCourseId ? 'Update Course' : 'Add Course'}</button>
              </div>
            </form>
          </div>
        </div>
